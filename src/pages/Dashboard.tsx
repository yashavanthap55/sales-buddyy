...
const Dashboard = () => {
  ...
  const scoredLeads = leads.filter(lead => lead.lead_scores?.[0]?.total_score).length;
  const highQualityLeads = leads.filter(lead => 
    lead.lead_scores?.[0]?.total_score && lead.lead_scores[0].total_score >= 70
  ).length;
  const averageScore = scoredLeads > 0 
    ? leads
        .filter(lead => lead.lead_scores?.[0]?.total_score)
        .reduce((sum, lead) => sum + (lead.lead_scores?.[0]?.total_score || 0), 0) / scoredLeads
    : 0;
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">Start by adding your first lead to begin the sales process</p>
              <Button onClick={() => navigate('/leads')}>
                <Plus className="h-4 w-4 mr-2" />
                Add Your First Lead
              </Button>
                ...
                <div key={lead.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center space-x-4">
                    ...
                    {lead.lead_scores?.[0]?.total_score && (
                      <div className={`text-sm font-medium ${getScoreColor(lead.lead_scores[0].total_score)}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate('/company-setup')}>
          <CardHeader>
          ...
          <CardContent>
            <p className="text-sm text-gray-600">Configure your company profile and product catalog</p>
          </CardContent>
        </Card>

        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate('/lead-qualification')}>
          <CardHeader>
            <CardTitle className="flex items-center">
          ...
          <CardContent>
            <p className="text-sm text-gray-600">Qualify and enrich your leads with detailed insights</p>
          </CardContent>
        </Card>
        ...
        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate('/analytics')}>
          ...
          <CardContent>
            <p className="text-sm text-gray-600">Track performance and conversion metrics</p>
          </CardContent>
        </Card>
      </div>
    </div>
